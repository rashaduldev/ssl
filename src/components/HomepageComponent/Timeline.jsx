...
//         </div>
//       ))}
//       <div className="absolute md:-top-[2%] top-0">
//       <div className="flex items-center space-x-4">
//       {/* SSL Circle with Gradient Border */}
//       <div
//         className="flex items-center justify-center w-20 h-20 rounded-full"
//         style={{
//           background: 'conic-gradient(blue, red, gray, yellow)', 
//           padding: '4px',
//         }}
//       >
...
// eslint-disable-next-line react/prop-types
const TimelineItem = ({ color, title, description, index, alignment }) => (
  <div
    className={`flex items-start mb-8 relative ${
      alignment === 'right' ? 'md:ml-[50%]' : 'md:mr-[50%]'
    }`}
  >
    {/* Timeline Content */}
    <div
      className={`flex items-center ${
        alignment === 'right' ? 'justify-between' : 'flex-row-reverse'
      } w-full`}
    >
      {/* SSL Circle */}
      <div
        className={`flex-shrink-0 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-gray-100 border border-blue-500 rounded-full text-xs font-semibold text-gray-600 shadow-md`}
      >
        SSL
      </div>
      ...
      {/* Title and Number */}
      <div
      className={`${alignment === 'right' ? 'text-left' : 'text-right'}`}
    >
    <div 
      className={`flex items-center ${
    ...
    >
      {/* Adjust the title and number */}
      <div
        className={`w-5 h-5 md:w-6 md:h-6 rounded-full ${color} flex items-center justify-center text-xs font-bold text-white`}
      >
        {String(index + 1).padStart(2, '0')}
      </div>
      <h3 className={`ml-2 text-sm md:text-lg font-semibold text-gray-700`}>
        {title}
      </h3>
    </div>
...
const Timeline = () => (
  <div className="px-4 md:px-0">
    ...
    <h1 className="text-red-700 text-center text-2xl italic my-10">Work in progress</h1>

    <div className="relative max-w-4xl mx-auto py-8 md:py-16">
      {/* Left Vertical Line */}
      <div className="absolute left-6 md:left-1/2 transform md:-translate-x-1/2 top-0 bottom-0 border-l-2 border-gray-300"></div>
      ...
      {items.map((_, index) => (
        <div
          key={index}
          className="absolute left-6 md:left-1/2 transform md:-translate-x-1/2 flex space-x-1"
          style={{ top: `${(index / items.length) * 100}%` }}
        >
          <div className="w-4 h-4 md:w-5 md:h-5 rounded-full bg-white">
            <FaRegCircle />
          </div>
        </div>
      ))}
